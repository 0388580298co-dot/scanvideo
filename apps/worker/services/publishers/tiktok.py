...
class TikTokPublisher:
    ...
    def creator_info(self) -> dict:
        response = requests.post(f"{API}/post/publish/creator_info/query/", headers={"Authorization": f"Bearer {self.access_token}", "Content-Type": "application/json"}, timeout=30)
    ...
    def publish_file(self, video_path: Path, title: str, privacy_level: str = "SELF_ONLY") -> dict:
        ...
        payload = {"post_info": {"title": title[:2200], "privacy_level": privacy_level}, "source_info": {"source": "FILE_UPLOAD", "video_size": size, "chunk_size": chunk, "total_chunk_count": total}}
        response = requests.post(f"{API}/post/publish/video/init/", headers={"Authorization": f"Bearer {self.access_token}", "Content-Type": "application/json"}, json=payload, timeout=30)
        response.raise_for_status()
