...
def upgrade() -> None:
    ...
    op.create_table("jobs", sa.Column("id", sa.String(64), primary_key=True), sa.Column("source_url", sa.Text(), nullable=False), sa.Column("status", sa.String(32), nullable=False), sa.Column("target_language", sa.String(10), nullable=False), sa.Column("progress", sa.Integer(), nullable=False, server_default="0"), sa.Column("message", sa.Text(), nullable=False, server_default=""), sa.Column("output_path", sa.Text()), sa.Column("error", sa.Text()), sa.Column("auto_publish", sa.Boolean(), nullable=False, server_default=sa.false()), sa.Column("created_at", sa.DateTime(timezone=True), nullable=False), sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False))
    ...
    op.create_index("ix_source_media_fingerprint", "source_media", ["fingerprint"])
    ...
    op.create_table("rendered_media", sa.Column("id", sa.Integer(), primary_key=True), sa.Column("job_id", sa.String(64), sa.ForeignKey("jobs.id", ondelete="CASCADE"), nullable=False), sa.Column("path", sa.Text(), nullable=False), sa.Column("width", sa.Integer(), nullable=False), sa.Column("height", sa.Integer(), nullable=False), sa.Column("duration", sa.Float(), nullable=False), sa.Column("qc_passed", sa.Boolean(), nullable=False, server_default=sa.false()), sa.Column("qc", sa.JSON(), nullable=False))
    op.create_index("ix_rendered_media_job_id", "rendered_media", ["job_id"])
    op.create_table("platform_accounts", sa.Column("id", sa.Integer(), primary_key=True), sa.Column("platform", sa.String(32), nullable=False), sa.Column("account_name", sa.String(160), nullable=False), sa.Column("credential_ref", sa.String(255)), sa.Column("enabled", sa.Boolean(), nullable=False, server_default=sa.true()))
    op.create_index("ix_platform_accounts_platform", "platform_accounts", ["platform"])
    op.create_table("scheduled_posts", sa.Column("id", sa.Integer(), primary_key=True), sa.Column("job_id", sa.String(64), sa.ForeignKey("jobs.id", ondelete="CASCADE"), nullable=False), sa.Column("platform_account_id", sa.Integer(), sa.ForeignKey("platform_accounts.id", ondelete="CASCADE"), nullable=False), sa.Column("scheduled_at", sa.DateTime(timezone=True), nullable=False), sa.Column("status", sa.String(32), nullable=False, server_default="SCHEDULED"))
    ...
    op.create_table("trend_items", sa.Column("id", sa.Integer(), primary_key=True), sa.Column("title", sa.Text(), nullable=False), sa.Column("source_url", sa.Text(), nullable=False, unique=True), sa.Column("score", sa.Float(), nullable=False, server_default="0"), sa.Column("source", sa.String(64), nullable=False), sa.Column("duration", sa.Float()), sa.Column("discovered_at", sa.DateTime(timezone=True), nullable=False))
    op.create_index("ix_trend_items_discovered_at", "trend_items", ["discovered_at"])


def downgrade() -> None:
    for table in ["trend_items", "published_posts", "scheduled_posts", "platform_accounts", "rendered_media", "tts_segments", "translations", "transcripts", "source_media", "jobs", "users"]:
        op.drop_table(table)
